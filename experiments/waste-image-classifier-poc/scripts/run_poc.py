...
def main() -> int:
    ...
    run([sys.executable, "scripts/train.py", "--config", args.config, "--output-dir", frozen_run, "--mode", "frozen_backbone"])
    final_run = frozen_run
    if not args.skip_fine_tune:
        final_run = f"{frozen_run}_fine_tune"
        run([sys.executable, "scripts/train.py", "--config", args.config, "--output-dir", final_run, "--mode", "fine_tune", "--resume-checkpoint", f"artifacts/runs/{frozen_run}/best_model.pt"])
    checkpoint = f"artifacts/runs/{final_run}/best_model.pt"
    run([sys.executable, "scripts/evaluate.py", "--checkpoint", checkpoint])
    if not args.skip_external:
        run([sys.executable, "scripts/evaluate_external.py", "--checkpoint", checkpoint, "--temperature", f"artifacts/runs/{final_run}/temperature_scaling.json", "--threshold-policy", f"artifacts/runs/{final_run}/threshold_policy.json", "--output-dir", f"artifacts/runs/{final_run}/external_evaluation"])
    print("\nFinal summary")
