...
def build_clip_frozen_head(num_classes: int, hf_model_id: str = DEFAULT_HF_MODEL_ID):
    import torch
    ...
    class ClipFrozenHeadClassifier(torch.nn.Module):
        def __init__(self):
            ...
            self.vision_model = CLIPVisionModel.from_pretrained(hf_model_id)
...
def clip_metadata(config: dict, selected_hyperparameters: dict | None = None) -> dict:
    ...
    return {
        ...
        "preprocessing_identifier": f"CLIPImageProcessor::{model_cfg.get('hf_model_id', DEFAULT_HF_MODEL_ID)}",
