...
except Exception:  # optional plotting dependency may be absent in smoke-test environments
    sns = None
...
def plot_confusion_matrix(matrix, class_names: list[str], output_path: str | Path, normalized: bool = False) -> None:
    ...
    if sns is not None:
        ...
        plt.imshow(np.asarray(matrix), cmap="Blues")
        plt.xticks(range(len(class_names)), class_names, rotation=45, ha="right")
        plt.yticks(range(len(class_names)), class_names)
        for y, row in enumerate(np.asarray(matrix)):
            for x, value in enumerate(row):
                plt.text(x, y, f"{value:.2f}" if normalized else str(int(value)), ha="center", va="center")
