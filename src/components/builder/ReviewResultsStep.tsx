...
export function ReviewResultsStep({ pipeId, trainModelsOutput, initialReviewResultsOutput, onCompleted, onBackToTrainModels }: ReviewResultsStepProps) {
  ...
  return <div>
    ...
    <section className="mt-6 rounded-3xl border border-black/10 bg-white/60 p-6"><h2 className="text-lg font-semibold">Charts</h2><div className="mt-4 grid gap-4 lg:grid-cols-2">{content.charts.map((chart) => <figure key={chart.chart_key} className="rounded-2xl border border-black/10 bg-white p-4"><figcaption className="font-medium">{chart.title}</figcaption><img className="mt-3 w-full rounded-xl border border-black/5" src={`data:image/png;base64,${chart.image_base64}`} alt={chart.title} /><p className="mt-3 rounded-xl bg-black/5 px-3 py-2 text-sm font-medium text-black/70">{chart.description}</p></figure>)}</div></section>

    <section className="mt-6 rounded-3xl border border-black/10 bg-white/60 p-6"><h2 className="text-lg font-semibold">Prediction examples</h2><p className="mt-2 text-sm text-black/60">These are only 10 sample rows from the validation set, not every row used in the charts.</p><div className="mt-4 overflow-x-auto"><table className="min-w-full text-sm"><thead><tr>{content.prediction_examples.columns.map((column) => <th key={column} className="border-b border-black/10 px-3 py-2 text-left font-medium">{column}</th>)}</tr></thead><tbody>{content.prediction_examples.rows.map((row, idx) => <tr key={idx}>{content.prediction_examples.columns.map((column) => <td key={column} className="border-b border-black/5 px-3 py-2 text-black/70">{displayValue(row[column])}</td>)}</tr>)}</tbody></table></div></section>
