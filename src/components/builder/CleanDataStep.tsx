...
import { applyCleaningPlan, auditDataQuality, generateCleaningPlan, getAffectedRowsForColumn, type AppliedCleaning } from "../../lib/builder/cleanData";
...
function affectedRowColumns(rows: Record<string, unknown>[], affectedColumn: string) {
  const columns = Object.keys(rows[0] ?? {});
  const affectedColumnIndex = columns.indexOf(affectedColumn);
  const followingColumns = columns.slice(affectedColumnIndex + 1, affectedColumnIndex + 6);
  return [affectedColumn, ...followingColumns];
}
...
export function CleanDataStep({ pipeId, selectDatasetOutput, initialCleanOutput, onCompleted, onBackToSelectDataset }: CleanDataStepProps) {
  ...
  const [expandedAffectedColumn, setExpandedAffectedColumn] = useState<string | null>(null);
  ...
  return <div>
    ...
    <section className="mt-6 rounded-3xl border border-black/10 bg-white/60 p-6"><h2 className="text-lg font-semibold">Recommended cleaning plan</h2>{issueColumns.length === 0 && audit.duplicateRows === 0 ? <p className="mt-3 text-sm text-black/60">This dataset looks clean. MLP does not need to change anything before splitting.</p> : null}{audit.duplicateRows > 0 ? <div className="mt-4 rounded-2xl border border-black/10 p-4"><p className="font-medium">Duplicate rows</p><p className="mt-1 text-sm text-black/60">MLP found {audit.duplicateRows} duplicate rows and recommends removing them.</p><select value={cleaningPlan?.duplicateRows.action ?? "remove"} onChange={(event) => updateDuplicateAction(event.target.value as "remove" | "keep")} className="mt-3 rounded-xl border border-black/10 bg-white px-3 py-2 text-sm"><option value="remove">Remove duplicates — recommended</option><option value="keep">Keep duplicates</option></select></div> : null}<div className="mt-4 grid gap-4">{issueColumns.map((column) => {
      const affectedRows = getAffectedRowsForColumn(rows, column.name);
      const shownAffectedRows = affectedRows.slice(0, 20);
      const contextColumns = affectedRowColumns(rows, column.name);
      const isExpanded = expandedAffectedColumn === column.name;

      return <div key={column.name} className="rounded-2xl border border-black/10 p-4"><p className="font-medium">Column “{column.name}”</p><p className="mt-1 text-sm text-black/60">{column.missing_count > 0 ? `${column.missing_count} values look missing. ` : ""}{column.special_missing_token_count ? "Some values look missing even though they are stored as text. " : ""}{column.recommended_role === "exclude_from_features" ? "MLP recommends not using this column for training later." : ""}</p>{column.recommended_strategy ? <select value={column.selected_strategy} onChange={(event) => updateColumnStrategy(column.name, event.target.value as CleaningStrategy)} className="mt-3 rounded-xl border border-black/10 bg-white px-3 py-2 text-sm"><option value={column.recommended_strategy}>{strategyLabels[column.recommended_strategy]}</option><option value="most_frequent">Use most frequent value</option><option value="remove_rows">Remove rows with missing values</option><option value="leave_as_is">Leave as is</option>{column.detected_type === "number" ? <><option value="mean">Use mean</option><option value="median">Use median</option></> : null}</select> : null}{affectedRows.length > 0 ? <div className="mt-3"><button type="button" onClick={() => setExpandedAffectedColumn(isExpanded ? null : column.name)} className="rounded-full border border-black/15 px-3 py-1 text-xs font-medium text-black/70 transition hover:border-black/30">{isExpanded ? "Hide affected rows" : "View affected rows"}</button>{isExpanded ? <div className="mt-3 rounded-2xl bg-black/[0.03] p-3"><p className="text-xs text-black/60">Showing {shownAffectedRows.length} of {affectedRows.length} affected rows.</p><div className="mt-2 overflow-x-auto"><table className="min-w-full text-xs"><thead><tr><th className="border-b border-black/10 px-2 py-2 text-left font-medium">Row</th>{contextColumns.map((contextColumn) => <th key={contextColumn} className="border-b border-black/10 px-2 py-2 text-left font-medium">{contextColumn}</th>)}</tr></thead><tbody>{shownAffectedRows.map(({ rowIndex, row }) => <tr key={rowIndex}><td className="border-b border-black/5 px-2 py-2 text-black/50">{rowIndex + 1}</td>{contextColumns.map((contextColumn) => <td key={contextColumn} className={`border-b border-black/5 px-2 py-2 ${contextColumn === column.name ? "font-medium text-red-700" : "text-black/60"}`}>{displayRawValue(row[contextColumn])}</td>)}</tr>)}</tbody></table></div></div> : null}</div> : null}</div>;
    })}</div></section>
