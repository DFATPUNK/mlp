...
export function PipeBuilderPage() {
  ...
  const [pipeId, setPipeId] = useState<string | null>(routePipeId ?? null);
  ...
  useEffect(() => {
    if (!routePipeId) {
      setPipeId(null);
      setLoadedPipeType(null);
      setExistingStepOutput(null);
      setCleanDataOutput(null);
      setActiveStep("select_dataset");
      return;
    }
    let mounted = true;
  ...
  async function handleUseDataset() {
    ...
    try {
      ...
      if (result.pipeId && !routePipeId) navigate(`/app/pipes/${result.pipeId}/builder`, { replace: true });
    } catch {
