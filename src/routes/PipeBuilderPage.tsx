...
import { getChooseTargetStepOutput, getCleanDataStepOutput, getPipeById, getReviewResultsStepOutput, getSelectDatasetStepOutput, getSplitDataStepOutput, getTestPredictionStepOutput, getTrainModelsStepOutput, isReviewResultsAcknowledged, type ChooseTargetStepOutput, type CleanDataStepOutput, type ReviewResultsStepOutput, type SelectDatasetStepOutput, type SplitDataStepOutput, type TestPredictionStepOutput, type TrainModelsStepOutput } from "../lib/pipes";
...
export function PipeBuilderPage() {
  ...
  useEffect(() => {
    ...
    void Promise.all([getPipeById(pipeId), getSelectDatasetStepOutput(pipeId), getCleanDataStepOutput(pipeId), getSplitDataStepOutput(pipeId), getChooseTargetStepOutput(pipeId), getTrainModelsStepOutput(pipeId), getReviewResultsStepOutput(pipeId), getTestPredictionStepOutput(pipeId)]).then(([pipe, stepOutput, cleanOutput, splitOutput, targetOutput, trainOutput, reviewOutput, testOutput]) => {
      ...
      if (testOutput || isReviewResultsAcknowledged(reviewOutput)) setActiveStep("test_prediction");
  ...
  return <div className="grid gap-8 lg:grid-cols-[280px_1fr]"><aside className="rounded-3xl border border-black/10 bg-white/60 p-5"><p className="text-xs uppercase tracking-[0.2em] text-black/40">Builder steps</p><ol className="mt-4 space-y-3">{steps.map((step, idx) => { const isActive = (idx === 0 && activeStep === "select_dataset") || (idx === 1 && activeStep === "clean_data") || (idx === 2 && activeStep === "split_data") || (idx === 3 && activeStep === "choose_target") || (idx === 4 && activeStep === "train_models") || (idx === 5 && activeStep === "review_results") || (idx === 6 && activeStep === "test_prediction"); const isCompleted = (idx === 0 && !!existingStepOutput) || (idx === 1 && !!cleanDataOutput) || (idx === 2 && !!splitDataOutput) || (idx === 3 && !!chooseTargetOutput) || (idx === 4 && !!trainModelsOutput) || (idx === 5 && reviewAcknowledged) || (idx === 6 && !!testPredictionOutput); return <li key={step} className={`rounded-2xl px-3 py-2 text-sm ${isActive ? "bg-black text-white" : isCompleted ? "bg-emerald-500/10 text-emerald-800" : "text-black/60"}`}>{idx + 1}. {step}{isCompleted ? " — completed" : !isActive && idx > 6 ? " — coming soon" : ""}</li>; })}</ol></aside>
