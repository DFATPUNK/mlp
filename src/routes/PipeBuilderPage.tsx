...
import { getCleanDataStepOutput, getPipeById, getSelectDatasetStepOutput, getSplitDataStepOutput, type CleanDataStepOutput, type SelectDatasetStepOutput, type SplitDataStepOutput } from "../lib/pipes";
...
export function PipeBuilderPage() {
  ...
  const [activeStep, setActiveStep] = useState<"select_dataset" | "clean_data" | "split_data">("select_dataset");
  ...
  useEffect(() => {
    ...
    void Promise.all([getPipeById(pipeId), getSelectDatasetStepOutput(pipeId), getCleanDataStepOutput(pipeId), getSplitDataStepOutput(pipeId)]).then(([pipe, stepOutput, cleanOutput, splitOutput]) => {
      if (!mounted) return;
      ...
      if (splitOutput || cleanOutput) setActiveStep("split_data");
  ...
  return <div className="grid gap-8 lg:grid-cols-[280px_1fr]"><aside className="rounded-3xl border border-black/10 bg-white/60 p-5"><p className="text-xs uppercase tracking-[0.2em] text-black/40">Builder steps</p><ol className="mt-4 space-y-3">{steps.map((step, idx) => { const isActive = (idx === 0 && activeStep === "select_dataset") || (idx === 1 && activeStep === "clean_data") || (idx === 2 && activeStep === "split_data"); const isCompleted = (idx === 0 && !!existingStepOutput) || (idx === 1 && !!cleanDataOutput) || (idx === 2 && !!splitDataOutput); return <li key={step} className={`rounded-2xl px-3 py-2 text-sm ${isActive ? "bg-black text-white" : isCompleted ? "bg-emerald-500/10 text-emerald-800" : "text-black/60"}`}>{idx + 1}. {step}{isCompleted ? " — completed" : idx > 1 ? " — coming soon" : ""}</li>; })}</ol></aside>
    <div><Link to="/app/pipes" className="text-sm text-black/50 hover:text-black">← Back to pipes</Link><h1 className="mt-4 text-4xl font-semibold tracking-tight">{activeStep === "split_data" ? "Step 3: Split data" : activeStep === "clean_data" ? "Step 2: Clean data" : "Step 1: Select dataset"}</h1>{loadedPipeType ? <p className="mt-2 text-xs uppercase tracking-[0.16em] text-black/40">{loadedPipeType.replaceAll("_", " ")}</p> : null}<p className="mt-2 text-black/60">{activeStep === "split_data" ? "MLP will split your cleaned dataset into training, validation, and test sets before model training." : activeStep === "clean_data" ? "MLP checks your dataset for common issues and recommends safe fixes before training." : "Connect data, preview it, and confirm it is eligible for the MVP before continuing."}</p>
      ...
      {activeStep === "split_data" ? <SplitDatasetStep pipeId={pipeId} pipeType={loadedPipeType} cleanDataOutput={cleanDataOutput} initialSplitOutput={splitDataOutput} onCompleted={setSplitDataOutput} onBackToCleanData={() => setActiveStep("clean_data")} /> : null}
