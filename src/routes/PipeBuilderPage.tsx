...
import { getChooseTargetStepOutput, getCleanDataStepOutput, getPipeById, getReviewResultsStepOutput, getSelectDatasetStepOutput, getSplitDataStepOutput, getTrainModelsStepOutput, type ChooseTargetStepOutput, type CleanDataStepOutput, type ReviewResultsStepOutput, type SelectDatasetStepOutput, type SplitDataStepOutput, type TrainModelsStepOutput } from "../lib/pipes";
...
export function PipeBuilderPage() {
  ...
  const [activeStep, setActiveStep] = useState<"select_dataset" | "clean_data" | "split_data" | "choose_target" | "train_models" | "review_results">("select_dataset");
  ...
  useEffect(() => {
    ...
    void Promise.all([getPipeById(pipeId), getSelectDatasetStepOutput(pipeId), getCleanDataStepOutput(pipeId), getSplitDataStepOutput(pipeId), getChooseTargetStepOutput(pipeId), getTrainModelsStepOutput(pipeId), getReviewResultsStepOutput(pipeId)]).then(([pipe, stepOutput, cleanOutput, splitOutput, targetOutput, trainOutput, reviewOutput]) => {
      ...
      if (reviewOutput || trainOutput) setActiveStep("review_results");
  ...
  return <div className="grid gap-8 lg:grid-cols-[280px_1fr]"><aside className="rounded-3xl border border-black/10 bg-white/60 p-5"><p className="text-xs uppercase tracking-[0.2em] text-black/40">Builder steps</p><ol className="mt-4 space-y-3">{steps.map((step, idx) => { const isActive = (idx === 0 && activeStep === "select_dataset") || (idx === 1 && activeStep === "clean_data") || (idx === 2 && activeStep === "split_data") || (idx === 3 && activeStep === "choose_target") || (idx === 4 && activeStep === "train_models") || (idx === 5 && activeStep === "review_results"); const isCompleted = (idx === 0 && !!existingStepOutput) || (idx === 1 && !!cleanDataOutput) || (idx === 2 && !!splitDataOutput) || (idx === 3 && !!chooseTargetOutput) || (idx === 4 && !!trainModelsOutput) || (idx === 5 && !!reviewResultsOutput); return <li key={step} className={`rounded-2xl px-3 py-2 text-sm ${isActive ? "bg-black text-white" : isCompleted ? "bg-emerald-500/10 text-emerald-800" : "text-black/60"}`}>{idx + 1}. {step}{isCompleted ? " — completed" : !isActive && idx > 5 ? " — coming soon" : ""}</li>; })}</ol></aside>
    <div><Link to="/app/pipes" className="text-sm text-black/50 hover:text-black">← Back to pipes</Link><h1 className="mt-4 text-4xl font-semibold tracking-tight">{activeStep === "review_results" ? "Step 6: Review results" : activeStep === "train_models" ? "Step 5: Train models" : activeStep === "choose_target" ? "Step 4: Choose what to predict" : activeStep === "split_data" ? "Step 3: Split data" : activeStep === "clean_data" ? "Step 2: Clean data" : "Step 1: Select dataset"}</h1>{loadedPipeType ? <p className="mt-2 text-xs uppercase tracking-[0.16em] text-black/40">{loadedPipeType.replaceAll("_", " ")}</p> : null}<p className="mt-2 text-black/60">{activeStep === "review_results" ? "Review the recommended model, metrics, charts, and validation examples from real training results." : activeStep === "train_models" ? "MLP will train and compare real baseline models, then recommend the best one." : activeStep === "choose_target" ? "Choose the column your pipe should learn to predict. MLP will detect whether this is a classification or regression problem." : activeStep === "split_data" ? "MLP will split your cleaned dataset into training, validation, and test sets before model training." : activeStep === "clean_data" ? "MLP checks your dataset for common issues and recommends safe fixes before training." : "Connect data, preview it, and confirm it is eligible for the MVP before continuing."}</p>
      ...
      {activeStep === "review_results" ? <ReviewResultsStep pipeId={pipeId} trainModelsOutput={trainModelsOutput} initialReviewResultsOutput={reviewResultsOutput} onCompleted={setReviewResultsOutput} onBackToTrainModels={() => setActiveStep("train_models")} /> : null}
